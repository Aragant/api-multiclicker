...
google_sso = GoogleSSO(
    ...
    f"{os.getenv('HOST')}/google/callback"
)
...
async def google_callback(request: Request):
    ...
    try:
        ...
        access_token = create_access_token(data={"sub": user_stored.id}, expires_delta=timedelta(minutes=int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES"))))
        
        request_ip = request.client.host
        request_user_agent = request.headers.get("User-Agent")
        
        refresh_token = create_refresh_token(
            data={"sub": user_stored.id}, expires_delta=timedelta(days=int(os.getenv("REFRESH_TOKEN_EXPIRE_DAYS")))
        )
        
        await RefreshTokenRepository().save(RefreshToken(user_id=user_stored.id, refresh_token=refresh_token, ip_address=request_ip, user_agent=request_user_agent))
