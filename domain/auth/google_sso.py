...
async def google_callback(request: Request):
    try:
        ...
        response.set_cookie(
            ...
            value=access_token,
            httponly=True,
            secure=True,  # mettre False en local si pas en HTTPS
            samesite="none",
            max_age=60 * int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES")),
        )

        # On peut aussi mettre le refresh token si besoin
        response.set_cookie(
            key="refresh_token",
            value=refresh_token,
            httponly=True,
            secure=True,
            samesite="none",
            max_age=60 * 60 * 24 * int(os.getenv("REFRESH_TOKEN_EXPIRE_DAYS")),
        )
